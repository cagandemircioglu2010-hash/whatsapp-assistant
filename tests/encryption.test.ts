...
describe("AES-256-GCM envelope encryption", () => {
  it("round-trips content with randomized nonces and authenticated purpose binding", () => {
    ...
    const first = encryption.encrypt("sensitive company message", "messages.content");
    const second = encryption.encrypt("sensitive company message", "messages.content");

    expect(first.ciphertext).not.toContain("sensitive company message");
    expect(first.ciphertext).not.toBe(second.ciphertext);
    expect(encryption.decrypt(first.ciphertext, "messages.content")).toBe("sensitive company message");
    expect(() => encryption.decrypt(first.ciphertext, "users.phone")).toThrow("authentication");
  });
  ...
  it("detects tampering and supports decrypting an older rotated key", () => {
    ...
    const protectedValue = oldEncryption.encrypt("+905551234567", "users.phone");
    const rotated = new EnvelopeEncryption(
      parseDataEncryptionConfig(JSON.stringify({ current: encoded(3), old: encoded(2) }), "current")
    );

    expect(rotated.decrypt(protectedValue.ciphertext, "users.phone")).toBe("+905551234567");
    ...
    expect(() => rotated.decrypt(tampered, "users.phone")).toThrow("authentication");
  });
