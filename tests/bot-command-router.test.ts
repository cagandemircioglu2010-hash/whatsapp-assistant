...
describe("bot command router", () => {
  ...
  it("does not treat a phrase embedded inside another word as a command", async () => {
    const { next, router } = build("en");
    const response = await router.handle(user, "show my database status", context);
    expect(response.text).toBe("downstream");
    expect(next.calls).toBe(1);
  });
});
