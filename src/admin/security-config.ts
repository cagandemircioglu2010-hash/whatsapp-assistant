...
import { parseDataEncryptionConfig } from "../security/encryption.js";
import {
  legacyHmacKeyRing,
  parseHmacKeyRing,
...
export function loadAdminSecurityConfig(
  ...
  if (!activeEncryptionKeyId || !encryptionKeys) {
    throw new Error("Data encryption keys must be set");
  }
  return {
    ...
    auditIntegrity: hmacRing(
      env,
      "AUDIT_INTEGRITY_ACTIVE_KEY_ID",
